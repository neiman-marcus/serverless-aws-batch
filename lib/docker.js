...
function buildDockerImage() {
    ...
    let runtime = this.serverless.service.provider.runtime;
    if (this.serverless.service.custom
        && this.serverless.service.custom.awsBatch
        && this.serverless.service.custom.awsBatch.runtime) {

        runtime = this.serverless.service.custom.awsBatch.runtime;
    }
    ...
    FROM justinram11/lambda:build-${runtime} AS builder
    USER root
    ...
    FROM justinram11/lambda:${runtime}
    ...
    if (runtime === 'nodejs8.10') dockerFileContents.concat(
    ...
    if (runtime === 'nodejs10.x') dockerFileContents.concat(
